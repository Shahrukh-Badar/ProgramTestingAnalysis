...
(function (sandbox) {
    ...
    function MyAnalysis() {
		...
		var dictObjectIdMap = new Object();
		var logProp = [];
		var countDelProp = 0;
		var countAddProp = 0;
		var objIId = 0;
        ...
        this.getField = function (iid, base, offset, val, isComputed, isOpAssign, isMethodCall) {
			if (offset == "prototype")
			{
				idProto  = sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(val));
				//console.log("getField " +idProto);
			}
            return {result: val};
        };
        ...
        this.putFieldPre = function (iid, base, offset, val, isComputed, isOpAssign) {
			...
			objIId = iid;
			objId  = sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(base));
			try{
			objAdd = base.hasOwnProperty(offset);
			}
			catch(ex)
			{
			...
			if(typeof(base) == 'object'){
				if(base.hasOwnProperty(offset) && typeof(val) == 'function'){
					//console.log(typeof(base));
					//console.log(base.constructor.name + ' override ' + offset + '  ,val:' + typeof(val)); 
					countOverride++;
				}
			}
			//console.log('putFieldPre:  ' +  objIId + ' ' + objId + ' ' + objAdd);
        ...
        this.putField = function (iid, base, offset, val, isComputed, isOpAssign) {
			//console.log('putField:  ' +  objIId + ' ' + objId + ' ' + objAdd);
			var id = sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(base));
			//console.log(base.hasOwnProperty(offset))
			if(typeof id !== "undefined")
			{
				var objName = dictObjectIdMap[id];
				if(typeof objName !== "undefined")
				{	
					if(	objIId == iid && objId == id && objAdd == false){
						//console.log('putField:  ' +  objIId + ' ' + objId + ' ' + objAdd);
						countAddProp++;
						//logProp.push("Added property '" + offset + "' value: '" + val +"' to object '" + objName + "'.");
					}
					else{
						//logProp.push("Updated property '" + offset + "' value: '" + val +"' to object '" + objName + "'.");
					}
					//console.log('objName: ' + objName);
				}
				else
				{
					//console.log('objName undefined against id: ' + id);
				}
				
				//Prototype update counter
				if (id == idProto)
					countProtoUpd++;
			...
			{
				//console.log('sandbox.smemory id undifined');
			}
			objIId = 0;
			objId = 0; 
			objAdd = null;
        ...
        this.write = function (iid, name, val, lhs, isGlobal, isScriptLocal) {
			...
			var id= sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(val));
			if(typeof id !== "undefined" && typeof val == 'object')
			{
				dictObjectIdMap[id] = name;
			}
			/*
        ...
        this.binary = function (iid, op, left, right, result, isOpAssign, isSwitchCaseComparison, isComputed) {
			/*if(typeof val == 'object'){
				console.log("cal is an object true :");
			}*/
			var id= sandbox.smemory.getIDFromShadowObjectOrFrame(sandbox.smemory.getShadowObjectOfObject(left));
			if(typeof id !== "undefined"  && op == 'delete')
			{
				var objName = dictObjectIdMap[id];
				if(typeof objName !== "undefined")
				{	
				    countDelProp++;
					//logProp.push("Deleted property '" + right + "' from object '" + objName + "'.");
					//console.log('objName: ' + objName);
				}
				else
				{
        ...
        this.conditional = function (iid, result) {
			/*
			//console.log('Method conditional');
			//console.log(result);
        ...
        this.instrumentCodePre = function (iid, code, isDirect) {
			...
			if(isDirect)
			{
				//tempLogs[countEval] = "Eval Found, Code: " + getValue(code);
				//console.log(isDirect);
				//console.log("Eval Found, Code: " + code);
				countEval++;
        ...
        this.endExecution = function () { 
			...
			/*tempLogs.forEach(function(element) {
			  console.log(element);
			});*/ //to print all eval
			console.log("---------------------------------------------");
			console.log("Total number of eval() found: " + countEval);
			console.log("---------------------------------------------");
			console.log("****************************************************************");	
			console.log("");	
			 //console.log(dictObjectIdMap);
			console.log("**********Dynamic Property Addition and Deletion**************");
			 /*logProp.forEach(function(element) {
			  console.log(element);
			});*/ // to print all addition and deletion stats
			console.log("---------------------------------------------");
			console.log("Total number of properties added: " + countAddProp);
			console.log("Total number of properties deleted: " + countDelProp);
			console.log("---------------------------------------------");
			console.log("****************************************************************");	
			console.log("");	
			console.log("**********Prototype Update Count**************");
			console.log("---------------------------------------------");
			console.log("Prototype Update Count: " + countProtoUpd);
			console.log("---------------------------------------------");
			console.log("****************************************************************");
			console.log("");	
			console.log("**********Method override Count**************");
			console.log("---------------------------------------------");
			console.log("Method override Count: " + countOverride);
			console.log("---------------------------------------------");
			console.log("****************************************************************");
			...
			//console.log('endExecution conditional');
